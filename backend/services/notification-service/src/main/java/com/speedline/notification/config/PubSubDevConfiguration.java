...
public class PubSubDevConfiguration {
    ...
    @Value("${spring.cloud.gcp.pubsub.subscriber.parallel-pull-count:1}")
    private Integer parallelPullCount;

    @Value("${spring.cloud.gcp.pubsub.subscriber.max-ack-extension-period:600}")
    private Long maxAckExtensionPeriodSeconds;
     ...
     * both publisher and subscriber paths use the production gRPC channels.
     */
    @Bean
    ...
    public SubscriberFactory subscriberFactory(
        ...
        factory.setParallelPullCount(parallelPullCount);
        factory.setMaxAckExtensionPeriod(Duration.ofSeconds(maxAckExtensionPeriodSeconds));
