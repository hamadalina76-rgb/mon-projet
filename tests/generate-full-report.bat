...
REM Usage: double-click generate-full-report.bat
REM =============================================================================
...
REM =============================================================================
REM Step 1: Check Docker is running + start services if needed
REM =============================================================================
...
REM Run Maven tests (skip integration/E2E tests)
...
for /d %%S in (services\*) do (
    if exist "%%S\target\allure-results" (
        ...
        for %%F in ("%%S\target\allure-results\*.json") do (
            copy /y "%%F" "%SCRIPT_DIR%\allure-results-unified\" >nul 2>&1
        )
    )
)
...
cd /d "%SCRIPT_DIR%"
node -e "const fs=require('fs'),path=require('path');const dir='allure-results-unified';fs.readdirSync(dir).filter(f=>f.endsWith('.json')).forEach(f=>{try{const p=path.join(dir,f);const d=JSON.parse(fs.readFileSync(p,'utf8'));if(!d.labels)d.labels=[];const hasSuite=d.labels.some(l=>l.name==='parentSuite');if(!hasSuite){d.labels.push({name:'parentSuite',value:'Unit Tests'});fs.writeFileSync(p,JSON.stringify(d))};}catch(e){}})" 2>nul
echo.
...
REM Copy and label API test results
for %%F in (allure-results\*.json) do (
    copy /y "%%F" "%SCRIPT_DIR%\allure-results-unified\" >nul 2>&1
)
node -e "const fs=require('fs'),path=require('path');const dir='allure-results';fs.readdirSync(dir).filter(f=>f.endsWith('.json')).forEach(f=>{try{const src=path.join(dir,f);const dst=path.join('%SCRIPT_DIR:\=/%/allure-results-unified',f);const d=JSON.parse(fs.readFileSync(src,'utf8'));if(!d.labels)d.labels=[];d.labels.push({name:'parentSuite',value:'API/Integration Tests'});fs.writeFileSync(dst,JSON.stringify(d));}catch(e){}})" 2>nul
echo.

REM =============================================================================
REM Layer 3: Playwright E2E Tests (Admin Panel + Partner Dashboard)
REM =============================================================================
echo [Layer 3/4] Running E2E tests (browser)...

REM Wait for frontends
echo   Checking Admin Panel (localhost:4200)...
:WAIT_ADMIN
curl -sf http://localhost:4200 >nul 2>&1
if errorlevel 1 (
    echo   Admin Panel not ready, waiting...
    timeout /t 5 /nobreak >nul
    goto WAIT_ADMIN
)
echo   Admin Panel is UP!

echo   Checking Partner Dashboard (localhost:4201)...
:WAIT_PARTNER
curl -sf http://localhost:4201 >nul 2>&1
if errorlevel 1 (
    echo   Partner Dashboard not ready, waiting...
    timeout /t 5 /nobreak >nul
    goto WAIT_PARTNER
)
echo   Partner Dashboard is UP!

REM Clear previous results and run E2E
if exist allure-results rd /s /q allure-results
...
REM Copy and label E2E results
for %%F in (allure-results\*.json) do (
    copy /y "%%F" "%SCRIPT_DIR%\allure-results-unified\" >nul 2>&1
)
node -e "const fs=require('fs'),path=require('path');const dir='allure-results';fs.readdirSync(dir).filter(f=>f.endsWith('.json')).forEach(f=>{try{const src=path.join(dir,f);const dst=path.join('%SCRIPT_DIR:\=/%/allure-results-unified',f);const d=JSON.parse(fs.readFileSync(src,'utf8'));if(!d.labels)d.labels=[];d.labels.push({name:'parentSuite',value:'E2E Tests'});fs.writeFileSync(dst,JSON.stringify(d));}catch(e){}})" 2>nul
echo.
...
if errorlevel 1 (
    echo   Allure generate failed. Falling back to Playwright HTML report...
